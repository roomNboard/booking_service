...
router.post('/:id', async (req, res, next) => {
  try {
    ...
    if ((await redisClient.clientGet.hexists(listingId, 'listingId')) === 1) {
      ...
      if (validateBooking(bookings, booking)) {
        console.log(await (await preDb).query(`SELECT MAX(the_primary_key) FROM the_table;`));
        console.log(await (await preDb).query(`SELECT nextval('the_primary_key_sequence');`));
        await db.insertBookingInfo(booking);
        res.sendStatus(201);
      } else {
        res.sendStatus(403);
